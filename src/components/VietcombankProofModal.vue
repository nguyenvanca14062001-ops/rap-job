...
const submitProof = async () => {
  ...
  try {
    ...
    const reportData = {
      ...
      // Thông tin chủ tài khoản Vietcombank do user tự nhập trong form (không tự điền từ hồ sơ web).
      fullName: name,
      phoneRef: phone,
    ...
    <div v-if="show" class="fixed inset-0 z-[5400] flex items-center justify-center p-4">
      ...
      <div class="relative bg-[#111726] border border-amber-500/30 w-full max-w-lg rounded-[36px] p-6 md:p-8 shadow-2xl max-h-[90vh] overflow-y-auto font-black italic uppercase text-left">
        <div class="flex items-center justify-between mb-6">
          <h2 class="text-lg md:text-xl text-white tracking-tight">📥 GỬI BẰNG CHỨNG VIETCOMBANK</h2>
        ...
        <div class="space-y-5">
          ...
          <div class="space-y-2">
            <label class="text-amber-400 text-[11px] tracking-widest ml-1">HỌ TÊN CHỦ TÀI KHOẢN VIETCOMBANK *</label>
            <input v-model="fullName" type="text" placeholder="Nhập họ tên đúng trên tài khoản Vietcombank"
          ...
          <div class="space-y-2">
            <label class="text-amber-400 text-[11px] tracking-widest ml-1">SĐT ĐĂNG KÝ VIETCOMBANK *</label>
            <input v-model="phoneRef" type="text" placeholder="Nhập SĐT đã dùng đăng ký Vietcombank"
                   class="w-full bg-[#0d121f] border border-slate-800 focus:border-amber-500 rounded-2xl py-3.5 px-5 text-white outline-none placeholder:text-slate-500 placeholder:normal-case font-sans not-italic font-semibold text-[14px] transition-colors" />
          </div>
