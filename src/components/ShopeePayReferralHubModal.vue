...
import ShopeePayReferralProofModal from '@/components/ShopeePayReferralProofModal.vue'
...
// Hướng dẫn copy nguyên từ job APP SHOPEE PAY gốc (src/data/jobs.ts -> 'shopee-pay'.quickSteps) — không bịa nội dung mới
const shopeePayJob = jobsData['shopee-pay']
const shopeePaySteps = shopeePayJob.quickSteps as any[]
const stepById = (id: number) => shopeePaySteps.find(s => s.id === id)
...
const shopeePayHistory = computed(() =>
  (props.myReports || [])
    .filter(r => r.jobId === SHOPEE_PAY_REFERRAL_JOB_ID)
    .slice()
  ...
  <Transition name="fade">
    <div v-if="show" class="fixed inset-0 z-[5200] flex items-center justify-center p-4">
      ...
      <div v-if="view === 'hub'" class="relative bg-gradient-to-br from-[#2A1C00] to-[#160E00] border border-amber-500/40 w-full max-w-md rounded-[36px] p-6 md:p-8 shadow-2xl text-center font-black italic uppercase">
        ...
        <h2 class="text-xl md:text-2xl text-white tracking-tighter leading-tight mb-2">GIỚI THIỆU BẠN BÈ<br/><span class="text-amber-400">ĐĂNG KÝ APP SHOPEE PAY</span></h2>
        <div class="bg-[#052e1f] border border-[#005c3c] rounded-full px-5 py-2 w-max mx-auto flex items-center gap-2 shadow-inner mb-6">
          ...
          <span class="text-[#00df89] text-[12px] md:text-sm tracking-tighter">THƯỞNG {{ SHOPEE_PAY_REFERRAL_REWARD.toLocaleString('vi-VN') }} XU / LƯỢT HỢP LỆ</span>
        </div>
        ...
        <div class="mt-5 bg-gradient-to-r from-orange-950/80 to-red-950/60 border-2 border-orange-500/60 rounded-2xl p-4 space-y-3 shadow-[0_0_25px_rgba(249,115,22,0.15)] text-left">
          <p class="text-orange-300 text-[13px] md:text-[15px] font-black normal-case leading-snug flex items-start gap-2">
            <span class="text-lg shrink-0">⚠️</span><span>1 điện thoại chỉ được đăng ký 1 tài khoản Shopee Pay duy nhất.</span>
          </p>
          <p class="text-orange-300 text-[13px] md:text-[15px] font-black normal-case leading-snug flex items-start gap-2">
            <span class="text-lg shrink-0">⚠️</span><span>1 CCCD/CMND chỉ được đăng ký 1 tài khoản Shopee Pay duy nhất.</span>
          </p>
      ...
      <!-- HƯỚNG DẪN (copy từ job APP SHOPEE PAY gốc) -->
      <div v-else-if="view === 'guide'" class="relative bg-[#111726] border border-slate-800 w-full max-w-lg rounded-[36px] p-6 md:p-8 shadow-2xl max-h-[85vh] overflow-y-auto text-left font-black italic uppercase">
        ...
        <h2 class="text-lg text-white tracking-tight mb-1">📖 HƯỚNG DẪN {{ shopeePayJob.title }}</h2>
        ...
        <div class="space-y-7">
          ...
          <div class="relative pl-9">
            ...
            <div class="mb-3 bg-[#1a0f14] border border-red-500/40 rounded-xl p-3.5 flex items-start gap-2 shadow-xl not-italic" v-if="stepById(2).note">
              <span class="text-red-500 text-base shrink-0">⚠️</span>
              <p class="text-red-400 text-[10px] font-black normal-case tracking-wide leading-relaxed">{{ stepById(2).note }}</p>
            </div>
            <div v-if="stepById(2).referralCode" class="mb-3 bg-[#0d121f] border border-emerald-500/50 p-3 rounded-2xl flex items-center gap-3 shadow-xl not-italic max-w-xs">
              <div class="flex-1 min-w-0">
                <p class="text-[9px] text-emerald-400 font-black tracking-[2px] uppercase mb-0.5 not-italic">MÃ GIỚI THIỆU</p>
                <p class="text-white text-lg font-black italic tracking-wider select-all truncate">{{ stepById(2).referralCode }}</p>
              </div>
              <button
                class="not-italic bg-emerald-500 hover:bg-emerald-400 text-[#090e17] px-4 py-2.5 rounded-xl text-[11px] font-black uppercase transition-all active:scale-95 shadow-lg shrink-0"
                @click="copyText(stepById(2).referralCode, 'Đã sao chép mã giới thiệu')">
                📋 SAO CHÉP
              </button>
            </div>
      ...
      <div v-else-if="view === 'history'" class="relative bg-[#111726] border border-slate-800 w-full max-w-lg rounded-[36px] p-6 md:p-8 shadow-2xl max-h-[85vh] overflow-y-auto text-left font-black italic uppercase">
        <div class="flex items-center justify-between mb-5">
          ...
          <h2 class="text-lg text-white tracking-tight">📜 LỊCH SỬ ĐƠN SHOPEE PAY</h2>
          <button @click="resetAndClose" class="w-8 h-8 rounded-xl bg-white/10 flex items-center justify-center text-slate-400 active:scale-90 transition-transform">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
          </button>
        </div>

        <div v-if="!shopeePayHistory.length" class="text-center py-12">
          <div class="text-4xl mb-3">📭</div>
          <p class="text-slate-500 text-[11px] normal-case font-bold leading-relaxed">Bạn chưa có đơn giới thiệu Shopee Pay nào.</p>
        </div>

        <div v-else class="space-y-3">
          <div v-for="rp in shopeePayHistory" :key="rp.id" class="bg-[#0d121f] border rounded-2xl p-4"
      ...
      <div v-if="showSuccess && lastSubmittedOrder" class="fixed inset-0 z-[5600] flex items-center justify-center p-4">
        ...
        <div class="relative bg-[#111726] border border-emerald-500/30 w-full max-w-sm rounded-[36px] p-7 text-center shadow-2xl font-black italic uppercase">
          ...
          <p class="text-slate-400 text-[10px] normal-case font-bold leading-relaxed mb-4">
            Đơn giới thiệu bạn bè APP SHOPEE PAY đã được gửi. Vui lòng chờ admin phê duyệt.
          </p>
      ...
      <ShopeePayReferralProofModal :show="showProof" @close="showProof = false" @submitted="handleSubmitted" />
    </div>
  </Transition>
</template>
