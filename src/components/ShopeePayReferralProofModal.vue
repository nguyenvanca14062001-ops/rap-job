...
// Ảnh mẫu lấy nguyên từ job SHOPEE PAY gốc (src/data/jobs.ts -> 'shopee-pay'.proofSampleImages) — cùng bộ ảnh, không tạo mẫu mới
const SAMPLE_IMAGES = jobsData['shopee-pay'].proofSampleImages as string[]
const MIN_IMAGES = SAMPLE_IMAGES.length
const MAX_IMAGES = SAMPLE_IMAGES.length
...
const submitReferral = async () => {
  ...
  try {
    ...
    try {
      // Upload + getDownloadURL của cả 2 ảnh chạy song song, không chờ tuần tự từng ảnh.
    ...
    const reportData = {
      ...
      jobId: SHOPEE_PAY_REFERRAL_JOB_ID,
      jobName: 'Giới thiệu bạn bè đăng ký SHOPEE PAY',
      title: 'GIỚI THIỆU BẠN BÈ SHOPEE PAY',
      ...
      reward: SHOPEE_PAY_REFERRAL_REWARD,
      actualReward: 0,
    ...
    <div v-if="show" class="fixed inset-0 z-[5400] flex items-center justify-center p-4">
      ...
      <div class="relative bg-[#111726] border border-amber-500/30 w-full max-w-lg rounded-[36px] p-6 md:p-8 shadow-2xl max-h-[90vh] overflow-y-auto font-black italic uppercase text-left">
        <div class="flex items-center justify-between mb-6">
          <h2 class="text-lg md:text-xl text-white tracking-tight">📥 GỬI BẰNG CHỨNG GIỚI THIỆU SHOPEE PAY</h2>
